from envs.One_bus_env import PowerSys_1bus
...
def train_agent(path,
    ...
    for episode in range(num_episodes):
        ...
        for step in range(num_steps):
            action,b = agent.policy_net.get_action(state)
            next_state, reward, done, _, penalty = env.step(action,b)
            ...
            th = state[0] #np.arccos(state[0]) * np.sign(state[1])
        ...
        print('Episode: ' + str(episode) + ' Reward: ' + str(episode_reward.detach().numpy()))
...
def run_agent(network_name):
    ...
    path = os.path.join(parent_dir, 'results/onebus' + str(seed) + '/' + network_name)
